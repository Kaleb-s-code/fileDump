...
#     Budget.myBase.metadata.create_all(Budget.myEngine)
    ...
    session.query(TheBudget).filter(TheBudget.itemName == theCategory).update(
        {TheBudget.budgetedValue: TheBudget.budgetedValue - theValue})
    session.commit()
    session.close()
...
#     Budget.myBase.metadata.create_all(Budget.myEngine)
    ...
    for item in budg:
        result += ('{0:<8}\t {1:<10}\t {2:<15}\t {3:<10}\t {4:<15}\t {5:<7}\t {6:<7}\n'.format(str(item.itemId), str(item.dateLastPaid), 
                       str(item.itemName), str(item.budgetedValue), str(item.expectedMonthlyValue), str(item.dueDate), str(item.itemNotes)))
