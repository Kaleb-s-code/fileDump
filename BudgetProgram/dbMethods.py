...
#     Budget.myBase.metadata.create_all(Budget.myEngine)
    ...
    elif theCategory == 'deduction': 
        anId = int(input("Which account would you like to deduct from?(Enter 0 for neither): "))
        if anId != 0:
...
#     Budget.myBase.metadata.create_all(Budget.myEngine)
    ...
    result += ("|{0:<8}|\t |{1:<10}|\t |{2:<15}\t |{3:<10}|\t |{4:<15}|\t |{5:<7}|\t |{6:<7}|\t\n".format(
        'itemId', 'dateLastPaid', 'itemName', 'itemValue', 'expectedMonthly', 'dueDate', 'notes'))
    for item in budg:
        result += ('{0:<8}\t {1:<10}\t {2:<15}\t {3:<10}\t {4:<15}\t {5:<7}\t {6:<7}\n'.format(str(item.itemId), str(item.dateLastPaid), 
                       str(item.itemName), str(item.itemValue), str(item.expectedMonthlyValue), str(item.dueDate), str(item.itemNotes)))
...
def getDbItemsIntoArray():
    ...
    result = []
    for item in budg:
        result.append([str(item.itemId), str(item.dateLastPaid), str(item.itemName), '${:,.2f}'.format(float(item.itemValue)), 
